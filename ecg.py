...
begin=10000           #setting start of interval to process
end=8000              #setting end of interval to process

ekg=pysqldf("SELECT * from file WHERE `sample interval`<"+str(begin)+" AND `sample interval`>"+str(end))
...
amount=len(peaks[0])
time=(pysqldf("SELECT max(`sample interval`) as bpm from file WHERE `sample interval`<"+str(begin)+" AND `sample interval`>"+str(end))-pysqldf("SELECT min(`sample interval`) as bpm from file WHERE `sample interval`<"+str(begin)+" AND `sample interval`>"+str(end)))*0.004
bps=amount/time
